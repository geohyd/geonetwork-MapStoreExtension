...
Extension load a geojson, and apply a default style.

Load list of layer available from the WPS.

Make an intersection or contains predicat between GeoJSON and all selected layers.

Open the Print module from a button in UI.

And add a transformer to the Print extension for adding the intersected feature info to the legend specs.

**This part is broken in epics.js :**
```
import {addTransformer} from '@mapstore/utils/PrintUtils';
addTransformer("kr-report-transfomer", (state, spec) => {
  //...
});
```
Seems to make a copy of `addTransformer` in extension and if change a local `let userTransformerChain = [];`

So, Print module does not have vairable but its own reference.



### README FROM SOURCE :

This repository is a based on template where to start to create [MapStore Extensions](https://mapstore.readthedocs.io/en/latest/developer-guide/extensions/).
...
`git clone --recursive https://github.com/geohyd/knowledgeReport-MapStoreExtension`
